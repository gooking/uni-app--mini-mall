			...
			<u-form ref="uForm" label-width="150rpx" :model="form">
				...
				<u-form-item label="短信验证码" prop="code" required>
					...
					<view slot="right" style="padding-left: 24rpx;">
						...
						<u-verification-code :seconds="seconds" @end="end" @start="start" ref="uCode" keepRunning @change="codeChange"></u-verification-code>
	...
	export default {
		...
		methods: {
			changeImgCode() {
				...
				this.imgsrc = this.$api.graphValidateCodeUrl(this.imgkey)
			},
			// 倒计时
			codeChange(text) {
				this.tips = text;
			...
			async getCode() {
				if(this.$refs.uCode.canGetCode) {
					...
					const res = await this.$api.smsValidateCode(this.form.mobile, this.imgkey, this.form.imgcode)
					if(res.code != 0) {
						uni.$u.toast(res.msg);
						return
					}
			...
			async submit() {
				...
				const res = await this.$api.resetPwdUseMobileCode(this.form.mobile, this.form.pwd, this.form.code)
				if(res.code == 10000) {
